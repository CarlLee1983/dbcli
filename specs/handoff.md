...
### ForgeFlowV2 的紀錄制度：目前是純成本

六天、23 個 Story、這份 handoff 五百多行。它到目前為止擋下的是**紀錄本身的錯**
（協議裡不存在的 `next_story: none`、CI 淺複製讓對帳靜默通過），不是程式的錯。這
種價值只有在「有人隔一段時間回來讀」或「第二個人接手」時才會兌現，而這兩件事都還
沒發生過。
...
繼續用，但知道自己在為什麼付費。ForgePilot 的部分不用猶豫。ForgeFlow 的紀錄制度
是一筆押注，不是已經兌現的收益。

**Falsified if:** 到 2026-12-08 為止，沒有任何一次決定是因為讀了 `specs/handoff.md`
或 `docs/adr/` 而改變的。屆時這套紀錄就是在給自己寫沒人看的報告，應該砍到只剩對帳
gate（`scripts/check-forgeflow-handoff.ts`、`scripts/check-forgeflow-adoption.ts`），
它們擋的是可查證的東西，成本也只有一次 CI。

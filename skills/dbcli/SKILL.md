...
| `semantic` | n/a | Validate, inspect drift, migrate to v2, or print the optional project-root `dbcli.semantic.json`. It is offline/read-only; v2 relationships can use only declared fields visible in cached schema, and metrics can reference only saved-query names. It never executes SQL. |

...
describe('the exemption ratchet', () => {
  ...
  test('admits nothing at all', () => {
    // The floor. Every Story passes upstream cleanly, so the gate now fails on
    // any finding rather than on any finding outside a list. The counts alone
    // would still agree if an entry were added and the numbers raised to match,
    // which is why the map itself is named empty here.
    expect([...PREDATING_FINDINGS.keys()]).toEqual([])
    expect(ADMITTED_FINDINGS).toBe(0)
    expect(ADMITTED_STORIES).toBe(0)
  })

...
| `semantic` | n/a | 驗證、搜尋、檢查漂移、遷移至 v2 或輸出可選的專案根目錄 `dbcli.semantic.json`。離線／唯讀；catalog search 只輸出受治理的 semantic metadata，v2 relationship 只能使用快取 schema 中可見、已宣告的欄位，metric 只能指向 saved-query 名稱；絕不執行 SQL。 |

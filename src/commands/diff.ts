...
// Moved to core by DBCLI-PLAT-011 so `impact assess` and `design` can import
// them without dragging this module's adapter imports into their static graph.
// Re-exported from here because that is where callers and tests already look.
import {
  loadOrmSchema,
  parseAgainstOrmValues,
  parseOrmFormat,
  type DriftOrmFormat,
} from '@/core/orm-drift/input'
...
export {
  ...
  parseAgainstOrmValues,
  type DriftOrmFormat,
  type LoadedOrmSchema,
  type LoadOrmSchemaOptions,
  type OrmAlias,
} from '@/core/orm-drift/input'

...
> ```
>
> Choose exactly one baseline. The command reads only the explicit design/ORM file or existing local cache, semantic contracts, saved-query names, verification artifact metadata, and optional reviewed `dbcli.data-access.json` operation metadata. That manifest must use canonical semantic references and existing workspace-relative source paths; dbcli never reads or parses those source files. It does not connect, refresh the cache, execute SQL, read query bodies, or publish protected identifiers. Missing, invalid, or redacted evidence is a visible `partial` coverage gap; v1 never reports complete coverage. `--fail-on` changes only the exit code (`error`, `warn`, or `never`) after the report has been written.

...
export function highlightSQL(sql: string): string {
  ...
  result = result.replace(KEYWORD_PATTERN, (match) =>
    pc.blue(pc.bold(match.toUpperCase()))
  )

  result = result.replace(NUMBER_PATTERN, (match) =>
    pc.yellow(match)
  )

  result = result.replace(/__STR_(\d+)__/g, (_, index) =>
    pc.green(strings[parseInt(index)])
  )

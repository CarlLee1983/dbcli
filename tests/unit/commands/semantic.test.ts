...
describe('semantic commands', () => {
  ...
  test('draft validate does not construct a database adapter', async () => {
    ...
    const createRulelessAdapterSpy = spyOn(AdapterFactory, 'createAdapterWithoutRules')
    ...
    try {
      ...
      expect(createRulelessAdapterSpy).not.toHaveBeenCalled()
      ...
      createRulelessAdapterSpy.mockRestore()

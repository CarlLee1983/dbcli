...
- `insert` calls `insertOne` with the JSON payload. The table-level blacklist and the column-level blacklist are checked before the connection opens — payloads touching blacklisted fields exit with `BlacklistError` and emit a JSON error. There is still **no** schema validation and **no** `--dry-run` (planned for PR2). Permission is gated through the synthetic statement `INSERT INTO dummy`, so the connection must have at least `read-write`.
- `update` accepts a JSON document or `key=value [AND key=value …]` in `--where`. The `--set` payload is automatically wrapped in `$set` unless any top-level key starts with `$` (e.g. `$inc`, `$push`, `$unset`). The table-level blacklist runs first, then the column-level blacklist runs against the keys inside `$set` / `$unset` (or the top-level keys for replacement-style payloads). Execution uses `updateMany`, so every matching document is updated. No `--dry-run` yet.
- `delete` requires `data-admin` or `admin` permission and runs `deleteMany`. The table-level blacklist is enforced before the connection opens. There is no `--dry-run` and no equivalent of "limit to one" — design filters carefully.

...
*   **黑名單管理器 (Blacklist Manager)**：從所有查詢結果中自動屏蔽敏感資料表與欄位。判定涵蓋語句參照到的**每一張**表，不只排在最前面那張：`JOIN`、逗號、`UNION` 或子查詢帶進來的黑名單表一樣會被擋下，`query` / `export` / `q` / `report` / shell 皆同。Redis saved query 與 report 診斷會在執行前解析 key target，並帶入該連線的 blacklist 與 mask 規則；`SCAN` evidence 也會移除受保護的 key 名稱。欄位規則與回傳欄位名比對時**整條點分路徑都不分大小寫**，所以 `SELECT Password AS PASSWORD` 繞不過寫成 `password` 的規則，規則 `profile.ssn` 也遮得住 `profile.SSN`——但改成完全不相干的別名仍然繞得過，欄位規則是顯示過濾而非存取控制這句話不受影響。折疊只發生在比對，設定檔裡的規則原樣保存；代價是 PostgreSQL 的 `"Password"` 與 `"password"`（或 MongoDB 同一份文件裡的 `profile.SSN` 與 `profile.ssn`）並存時，規則寫其中一個會兩個都遮。折疊本身不看上下文：`toLowerCase` 對希臘文大寫 `Σ` 在字尾與字中給出不同的小寫字母，所以 `ς` 與 `σ` 一律視為同一個字元，否則規則 `ΑΣ*` 會遮不住它自己指名的 `ΑΣ_num`。`--fields` 不受影響，維持精確比對。設定裡的前後空白與引號會被去掉（`" password "`、`"\"password\""`），以自己的表限定的規則（`{"users": ["users.password"]}`）現在會載入失敗而不是靜默不命中，寫在 `public.users` 底下的規則也會套用到 `SELECT * FROM users`，反向亦然。`blacklist.tables` 現在對所有引擎都是 glob，不再只有 Redis 與 Elasticsearch——`tables: ["secrets*"]` 會擋下 MongoDB collection `secrets_2026`，也會擋下 SQL 資料表 `secrets_2026`；這是 breaking change，真的叫 `report*` 的表要寫成 `report\*` 才能回到字面比對。遮罩以所有被參照的表的欄位規則聯集計算 —— JOIN 結果的欄位名不帶表限定，無法反推歸屬，因此從嚴。表名列舉刻意過度回報：語句中每個非保留字的識別字都算候選，所以欄位名或別名若與黑名單資料表同名，該語句會被擋（訊息會指出命中的名稱）。Elasticsearch 的 `--index` 是運算式（逗號清單、萬用字元、`_all`、date math、跨叢集限定），會正規化後逐一比對，萬用字元在可能命中黑名單 index 時拒絕；ES shell 依伺服器實際路由的路徑逐段檢查、檢查 request body 指名的 index（`_mget` / `_bulk` / `terms` lookup），並從回應中移除受保護欄位；無法界定 index 的請求（`GET /_search` 等）在有黑名單時一律拒絕。遮罩比對的是回傳欄位名，`SELECT password_hash AS x` 這類改名仍會回傳該值 —— **資料表層級的項目可強制執行，欄位層級的項目是顯示過濾，不是存取控制**。
...
capability 的 status、reason 與整體 `ok`。
...
**`dbcli shell` 只接第二級，不接第一級。** shell 裡的每一句都是人手打的，逐句 y/N 會變成反射動作；但沒有 WHERE 的 DELETE、DROP、TRUNCATE 一樣要打出資料表名稱。打錯只會印出「已取消」並回到提示符——連線、緩衝區與歷史都還在，行程不會結束。按 Ctrl-C 則是收回這個提問：什麼都不會執行，接下來打的那一行會被當成語句，而不是被吃成確認答案。以管線餵進來的輸入（`dbcli shell < script.sql`）沒有人能回答，該句會被拒絕，其餘的行照跑。Redis、MongoDB 與 Elasticsearch 的 shell 不走寫入閘門，改由連線的 permission 等級把關。**Elasticsearch 這一道在 4.0.0 之前並不存在** —— ES shell 完全沒有 permission 檢查就送到叢集，`query-only` 連線可以刪除文件、刪掉索引或改寫 mapping，而且什麼都沒被記錄。現在它用與 `dbcli query` 相同的分類器判斷每個請求，拒絕該等級不允許的操作，並且不論執行或被拒都寫入 audit —— 送出前一列、回應後一列，每列都以 `<METHOD> <path>` 寫明操作本身。伺服器端 script（`script`、`script_fields`）在 shell 裡也一律拒絕，與 `dbcli query` 一致：它們會在叢集上執行任意程式碼，並能把黑名單欄位以請求自選的名稱讀回來。以管線驅動的 shell 現在會把 audit 寫完才結束，`printf 'DELETE /orders\n\n' | dbcli shell` 不再能送到叢集卻不留紀錄；SQL shell 的同一個缺陷一併修正。**SQL shell 從 5.0.0 起為每一句語句寫稽核列**，形狀與 Elasticsearch shell 相同——送出前 `attempt`、回來後 `outcome`，被拒絕的語句只寫 `outcome`，因為它從未被嘗試。先前它只記錄 tier-two 的寫入閘門決策，所以在提示符打的 `SELECT` 與一句真的改了資料的 `UPDATE` 同樣不留痕跡，而同樣這兩句走 `dbcli query` 都會被記下。兩個 shell 的 phase 都放在 `metadata.shell_phase`。audit 預設是 best-effort——磁碟滿或 `.dbcli/audit` 不可寫時請求照樣送出，只留一行警告——若你認為「沒有紀錄的請求」比「被拒絕的請求」更糟，在設定裡開 `audit.strict: true`。它涵蓋 dbcli 在**執行之前**記錄操作的每一處：兩個 shell 送出前那一列，以及 SQL 寫入閘門的決定紀錄——現在也包含在 `dbcli>` 提示符打的 `SELECT`。事後才寫的紀錄不在範圍內——那時拒絕擋不回任何東西，只會把一次已完成的操作回報成失敗。`dbcli audit health` 看得到它現在是開是關。在 shell 裡以子指令形式打的 dbcli 指令（`query "..."`、`\delete ...`）跑在沒有 stdin 的獨立行程裡，沒有辦法提問，所以第二級語句會被拒絕，訊息會請你回到 `dbcli>` 提示符直接打。
...
[ADR-0013](https://github.com/CarlLee1983/dbcli/blob/main/docs/adr/ADR-0013-evidence-artifact-format-versions-are-independent-of-the-package-version.md)。
...
目前可用的指令是 `list`、`schema`、`query`、`q` 與 `status`。`init` 還沒有 SQLite 的精靈流程，請直接編輯 `config.json` 來設定連線。`insert`、`update`、`delete`、`export`、`migrate`、`diff`、`shell`、`doctor` 與 `report` 目前都還不支援 SQLite。
...
dbcli query "SCAN 0 MATCH secrets:*" # 拒絕(pattern 與規則重疊)
dbcli query "SCAN 0"                 # 照跑；受保護的 key 名從回應裡濾掉
dbcli delete secrets:api_key         # 拒絕 —— glob 涵蓋寫入，不只讀取

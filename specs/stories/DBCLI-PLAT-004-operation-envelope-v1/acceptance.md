...
* [x] `dbcli --agent-output capabilities check --require schema.read` emits one
...
* [x] Required/results/warnings caps are 128, evidence is capped at 16, identifier
      fields at 160 characters, messages at 2,000 characters, and the serialized
      envelope at 65,536 UTF-8 bytes — same file
* [x] An envelope at every exact boundary passes, while the next character,
      element, or byte fails closed without partial output — same file
* [x] Identical envelope inputs emit byte-identical compact JSON, stable
      top-level field order, and exactly one trailing newline — same file
* [x] A non-null recovery value passes the existing strict Recovery Envelope
...
* [x] Evidence references accept only `receipt`, `audit`, and
      ...
      file
* [x] Error and warning codes are bounded uppercase snake-case identifiers and
      their documented meanings remain locale-independent English — same file

## Failure Cases

* [x] Requirements unmet emits a failed envelope with bounded result data,
      `CAPABILITY_REQUIREMENTS_UNMET`, empty stderr, and exit `1` —
      `tests/integration/capabilities-command.test.ts`
* [x] Missing, empty, and empty-element `--require` each emit
      `INVALID_CAPABILITY_REQUIREMENTS`, `data: null`, empty stderr, and exit `2`
      — same file
* [x] Duplicate requirements are de-duplicated in first-seen order and mapped to
      `DUPLICATE_CAPABILITY_REQUIREMENT` without changing the result exit code —
      same file
* [x] Missing context, unresolvable context, and active agent-mode restrictions
      map to their three stable warning codes without leaking the underlying
      config error — same file
* [x] `--agent-output` placed after the subcommand, or combined with explicitly
      supplied `--format` or `--for-agent`, emits
      `INVALID_AGENT_OUTPUT_OPTIONS`, no prose, empty stderr, and exit `2` — same
      file
* [x] An unsupported ordinary command, `shell`, `es-shell`, `proxy`, `--help`,
...
* [x] Commander missing/malformed `--require` maps to
...
* [x] Command-tree load and unexpected action failures after opt-in each emit one
      `AGENT_OUTPUT_INTERNAL_ERROR` envelope with
      `Agent output failed safely.`, no raw error, empty stderr, and exit `1` —
      `tests/integration/lazy-entry-path.test.ts`
* [x] Field-level input overflow emits its input error and exit `2`; an otherwise
...
* [x] Before product source changes, exact success and requirements-unmet stdout,
      ...
      `tests/integration/capabilities-command.test.ts`
* [x] Existing `--format json`, text, Markdown, `--for-agent`, standalone
      ...
      `tests/integration/capabilities-command.test.ts`
* [x] Both envelope result paths remain offline and make no database connection
      or filesystem mutation — `tests/contract/capability-contract.test.ts` and
      `tests/integration/capabilities-command.test.ts`
* [x] English and Traditional Chinese user Markdown/HTML, root help,
...
* [x] Core no-stdout, CLI contract, package exports, lazy loading, plugin sync,

...
test('resolveKpi returns formatted currency value when rows[0] has the column', () => {
  ...
  expect(
    resolveKpi(rows, { label: 'Total', value_column: 'revenue', format: 'currency' })
  ).toBe('$1,000.00')
})
